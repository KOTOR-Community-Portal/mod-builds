...
[**KOTOR Modsync**](https://deadlystream.com/files/file/2317-kotormodsync/) uses TOML files which come with pre-set instructions which can automate the entire mod build install process--very handy. The installation is handled by a GUI that allows you as the user to pick and choose what mods in the builds you want to install, and all you need to do is download them beforehand.

However, there are a few caveats. If the TOML files are ever outdated (which can happen not only when this build is updated, but when any of the constituent mods of this build are updated by their authors), the install process will no longer function properly until the TOMLs are updated. The Modsync team does their best to keep everything up-to-date and we actively work with them to alert them of any changes to the builds, but for the size and number of these builds (6 Modsync-supported, 3 for each game) it's very difficult to keep them all functional all of the time. Additionally, while the GUI fully supports the removal of any build mods without any manual editing required, to add mods not present in the builds (or to use a different optional module of an included mod rather than the default, mod build-recommended module) requires manual editing of the instruction sets. These manual edits can be accomplished by you as the enduser, though it takes a certain level of tech-literacy to do so. Therefore, if the TOMLs are out-of-date or you want to add mods not present in the builds, Modsync is not necessarily the best option.

Because of this, while Modsync is an option you should definitely keep in mind, I generally recommend using it **if and only if** one of the following two scenarios applies: you trust my sense of balance & aesthetic preferences (IE you're willing to use all the content included here, installed as I recommend it), you're not very confident in installing these mods manually, AND KOTOR Modsync's TOML files are working properly; **OR** you are very tech-literate and comfortable manually editing these instruction sets to match all your desired parameters. In other words, if you're very tech-literate you can make KOTOR Modsync work wonders for you either way. If you're not and you want to add more mods to the build list/use different install options during the process OR the TOML files aren't functioning at present, it would likely be much faster for you to install things the old-fashioned way.
